...
pub static XSOVERLAY_SOCKET: LazyLock<Arc<tokio::sync::Mutex<Option<reqwest_websocket::WebSocket>>>> = LazyLock::new(|| Arc::new(tokio::sync::Mutex::new(None)));

pub async fn start_xsoverlay_socket(app: tauri::AppHandle<Wry>) -> Result<(), Box<dyn std::error::Error>> {
    ...
    tauri::async_runtime::spawn(async move {
        ...
        loop {
            ...
            let mut socket_guard = match timeout_result {
                Ok(guard) => guard,
                Err(_) => {
                    static LOCK_TIMEOUT_ATTEMPTS: std::sync::atomic::AtomicU32 = std::sync::atomic::AtomicU32::new(0);
                    let attempts = LOCK_TIMEOUT_ATTEMPTS.fetch_add(1, std::sync::atomic::Ordering::SeqCst) + 1;
                    if attempts >= 10 {
            ...
            if let Some(message) = timeout.ok().and_then(|res| res.ok()).and_then(|opt| opt) {
                match message {
                    Message::Text(text) => {
                        app.emit_to(EventTarget::App, "xsoverlay-response", text.clone()).unwrap();
                        println!("Received XSOverlay response: {:?}", text);
                    }
                    Message::Binary(bin) => {
                        // Ignore binary messages for now
                        println!("Received {}-byte binary message from XSOverlay, ignoring.", bin.len());
                    }
                    Message::Ping(bong) => {
                        // Respond to pings
                        let _ = socket_guard.as_mut().unwrap().send(Message::Pong(bong)).await;
                    ...
                    Message::Close { code, reason } => {
                        eprintln!("XSOverlay WebSocket connection closed: code={:?}, reason={:?}", code, reason);
                        *socket_guard = None;
                        break;
                    }
                    // _ => {
