...
use crate::{api::xsoverlay::XSOVERLAY_SOCKET, memory::advisories::AdvisoryMemory, types::{advisories::Notice, xsoverlay::{XSOverlayCommand, XSOverlayNotificationObject}}};
...
pub fn publish_notice(app: tauri::AppHandle<Wry>, notice: Notice) -> Result<(), String> {
    ...
    tauri::async_runtime::spawn(async move {
        ...
        if socket_guard.is_some() {
            ...
            let _ = socket_guard.as_mut().unwrap().send(reqwest_websocket::Message::Text(serde_json::to_string(&command).unwrap())).await
            .map_err(|e| eprintln!("Could not send XSOverlay notification: {:?}", e));
            let _ = socket_guard.as_mut().unwrap().flush().await.map_err(|e| eprintln!("Could not flush XSOverlay notifications: {:?}", e));
            // #[cfg(debug_assertions)]
            // println!("[DEBUG] Sent!");
            return;
        } else {
            ...
            Notification::new()
                ...
                .appname("VRCMRD")
                .show()
                .map_err(|e| eprintln!("Could not send desktop notification: {:?}", e)).ok();
            #[cfg(target_os = "windows")]
            Toast::new(Toast::POWERSHELL_APP_ID)
                ...
                .show()
                .map_err(|e| eprintln!("Could not send Windows toast notification: {:?}", e)).ok();
        }
