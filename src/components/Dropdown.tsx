...
export default function Dropdown({ items, class: className, label }: { items: { active: boolean, deprecated?: boolean, set: () => void, label: preact.VNode, description?: preact.VNode }[], class?: string, label?: preact.VNode }) {
  ...
  return <div class={`relative ${className ?? ""}`}>
    <Menu>
      ...
      <MenuItems anchor={{to: "bottom", padding: 24}} class="absolute bg-white dark:bg-gray-800 border border-gray-300 dark:border-gray-700 text-black dark:text-white rounded mt-1 w-full max-h-60 overflow-y-auto z-60">
        {items.filter(item => !item.deprecated).map((item, i) => <MenuItem key={i}>
          <div>
            <button class={menuItemClasses(item.active)} onClick={() => !item.active && item.set()}>
              {item.label}
              {item.description && <div class="text-sm text-gray-600 dark:text-gray-400">{item.description}</div>}
            </button>
          </div>
        </MenuItem>)}
      </MenuItems>
